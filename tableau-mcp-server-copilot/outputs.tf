...
output "mcp_proxy_url" {
  description = "MCP Proxy URL（Copilot Studio設定用）"
  value       = var.enable_proxy ? "http://${aws_eip.mcp_server.public_ip}:${var.proxy_server_port}/tableau-mcp" : "Proxy disabled"
}
